...
| 问题 | 根因 | 修复 |
|---|---|---|
| 导航不回退 | 在 `currentRowChanged` 处理器内直接 `setCurrentRow` 属嵌套修改，Qt 无法可靠回退选中 | 记录 `_active_row`，拒绝时 `blockSignals` 显式回退；并按流程补齐前置条件门（步骤3需镜像、步骤4需容器、步骤5需脚本） |
| 步骤栏出滚动条 | 200px 宽 + 高 DPI 缩放导致条目文字换行撑高 | 宽度 240px + `setWordWrap(False)` + 关横向滚动条 + `setUniformItemSizes` |
| 左侧无品牌 | — | 新增品牌区（logo + 名称 + 版本） |
| 手册调用外部程序 | `os.startfile` | 程序内 `QDialog + QTextBrowser` 只读显示（`find_manual_path` 兼容打包/源码） |
| 连接测试进度卡 50% | 任务函数只上报 50% 即结束 | 函数补 100%；面板在任务完成时自动补满/标记失败（系统性兜底） |
